...
module.exports = {
  ...
  callback: async ({ user, args }) => {
    ...
    const embed = messageToRoblox.MessageSend(stringifiedData, UNIVERSE_ID, TOPIC).then(async responseData => {
      ...
      if (!result) {
        return jobId? `UserId: ${userId} is not banned from server: ${jobId}` : `UserId: ${userId} is not server banned.`
      }

      return await ServerBan.deleteOne( jobId? {userId: userId, jobId: jobId} : {userId: userId} )
        .then(() => {
          // Return embed response
          return new discord.MessageEmbed()
          .setTitle(`Server unban user: ${userId}`)
          .setColor(responseData.success ? "GREEN" : "RED")
          .setDescription(responseData.success ? `Player prompted to be server unbanned` : "Unable to server unban the player")
